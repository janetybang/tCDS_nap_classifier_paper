...
# could get CDS predictions for all of CONTX..
all_contx <- read_csv(file="../data/allCONTX_wNaps.csv") %>% 
  filter(Sleep==0) 
# but we're missing some columns: noise_min, silence_min, meaningful_min, distant_min, tv_min, hi
...
# clustering/plotting done in 02-clustering.R
#knitr::include_graphics("clustered_segments.png") # can we get the PDF?
```
...
We now build a simple decision tree classifier to distinguish nap segments from waking segments, mirroring the first step that researchers often have to perform manually to clean the dataset.
We train the model using 5-fold cross-validation on 90% of the 12,931 segments (`r Nclass["nap"]` nap segments, `r nrow(d)-Nclass["nap"]` non-naps).
...
nap_dtree = readRDS(here("nap_classifier.Rds"))
nap_auc = round(nap_dtree$tree.roc$auc, 3)

knitr::include_graphics("nap_classifier.pdf")
```
...
Having found that raw LENA features can be effectively used to separate nap segments from non-nap segments, we now turn to the more challenging task of building a classifier to automatically distinguish CDS from non-CDS segments.
...
xgb_final <- readRDS("../final_rawLENA_xgb_model.Rds")
...
# with naps: AUC = .83 acc = .74
# without naps: AUC .746, acc = .675
dat$id = d_nonaps$id 

#library(cowplot)

#xgb = xgb_mean # meaningful-normed does better

col_names = attr(xgb$train.data, ".Dimnames")[[2]]
imp = xgb.importance(col_names, xgb$model)

# plot AUC and importance
pdf("rawLENA-xgboost-AUC-importance.pdf", width=8, height=4)
...
#  0 0.73 0.27
#  1 0.25 0.75
#cor.test(ttpreds$pred, ttpreds$propCTC) # .43
#cor.test(ttpreds$pred, ttpreds$propAWC) # -.05
#cor.test(ttpreds$pred, ttpreds$silence) # .46
#cor.test(ttpreds$pred, ttpreds$propCVC) # .27
...
high_conf_CDS <- subset(d_all, xgb_pred>0.7)
high_conf_ODS <- subset(d_all, xgb_pred<0.3)

table(high_conf_CDS$cds_ohs) / nrow(high_conf_CDS) # 2171 / 11057 (2884 for .7)
table(high_conf_ODS$cds_ohs) / nrow(high_conf_ODS) # 1564 / 11057 (2196 for .3)
...
# can we recover the same hourly AWC with our large dataframe?

### NEED TO ASSIGN SPLIT SEGMENTS 50% CDS / 50% ODS
#split_idx = which(d$cds_ohs=="split")
#d[split_idx,]$AWC = d[split_idx,]$AWC / 2
#dag_cds = d %>% filter(cds_ohs!=0) %>% group_by(id) %>% 
#  summarise(total_CDS = sum(AWC)) %>%
#  left_join(dag, by=c("id")) %>%
#  mutate(CDS_hr = total_CDS / total_duration_hrs) %>%
#  dplyr::select(-id)

# Adriana: Discrepancy is due to the “no media” segments, which we excluded but in your dataset are included as part of ODS. I don’t think it’s a huge deal because the correlations didn’t really change whether we excluded those segments or not, but if we want the numbers to match WF exactly then we would need to exclude them
# I’m attaching a file that has the segments we excluded because they were only media. It is the last variable in this file. But I think this file has some segments that are not in yours for other reasons (e.g., child was sleeping, it was a dif day, etc), so you will have to match the datasets based on id and Time.  In my file the Time variable that you want is called ClockTime. I also left the CDS, ODS, and split variables there so you can double check in case things still don’t match up. Let me know if you have any questions or want to chat
# These segments are included in the dataset you have; this dataset just indicates which ones should be excluded. These were segments that we originally marked as “child alone”, meaning that we could not hear any other live speaker in the segment, therefore the AWC estimated by LENA must have come from electronic media. Does that make sense?
...
#cor(WFdata$OHS_AWChr, WFdata$FreqOHShr) # .94
#plot(WFdata$OHS_AWChr, WFdata$FreqOHShr) # still a few weird OHS kids! naps? remove media..?

#cor(WFdata$CDS_AWChr, WFdata$FreqCDShr) # .999
#plot(WFdata$CDS_AWChr, WFdata$FreqCDShr) # perfect

#cor(WFdata$CDS_AWChr, WFdata$vocab24) # .52
#cor(WFdata$OHS_AWChr, WFdata$vocab24) # .17 <- our OHS calcs..
#cor(WFdata$FreqOHShr, WFdata$vocab24) # .25 <- WF paper
...
ggsave("WF_outcomes_hand-coded_vs_modelCDS.pdf", width=6, height=3)
...
ggarrange(p1, p2, nrow=1)
ggsave("WF_outcomes_hand-coded_vs_modelOHS.pdf", width=6, height=3)
...
ggpairs(mod_expected, columns= c("CTC","AWC","FreqCDShr","FreqOH_noMedia","expCDS_AWC","expODS_AWC","vocab24"), 
        ggplot2::aes(alpha=.5)) + 
  theme_bw()
ggsave("model_expectedCDS_vs_WForig.pdf", width=9, height=9)
```
...
### Additional CONTX Predictions?
Use the classifier to predict CDS / OHS for the rest of the CONTX dataset,
and correlate total (predicted) CDS vs. outcomes (CDI scores), and compare to correlation between raw AWC and outcomes.
